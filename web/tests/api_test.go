...
func TestWholeGameApi(t *testing.T) {
	...
	t.Run("TestUpdateContestStates", TestUpdateContestStates)
...
func TestGetContestant(t *testing.T) {
	...
	assert.Nil(t, err)
	assert.Equal(t, 200, result["status"].(int))
	assignAI := result["assigned_ai"].(map[string]interface{})
...
func TestUpdateContestStates(t *testing.T) {
	url := "http://localhost:8080/api/v1/games/3/states"
	method := "PATCH"
	payload := `{
	"assign_ai_enabled": true
}`
...
func TestRegisterContest(t *testing.T) {
	url := "http://localhost:8080/api/v1/contests/3/register"
	method := "PUT"
	payload := `{
	"password": ""
}`
